...
export function ChatView({ chat }: ChatViewProps) {
  ...
  const handleChatSubmit = useCallback<NonNullable<ChatInputProps["onSubmit"]>>(
    async ({ draft, role }) => {
      ...
      console.log(Object.values(chat.history).length);
      var last = Object.values(chat.history)[Object.values(chat.history).length-1];

      
      dispatch(pushHistory({ content: draft, role: role }));
      dispatch(updateDraft({ id: chat.id, draft: "" }));
      console.log(last.content)
      responseObj = await Axios.post('http://127.0.0.1:8000/answer', [last.content, draft])
      ...
      const resultString = await Promise.all(responseObj);
      const finalString = resultString.join("");
      dispatch(pushHistory({ content: finalString, role: "assistant" }));
    },
    [chat, dispatch]
  );
